...
export const PortfolioTable = (props: PortfolioTableProps) => {
  ...
  const sortBy = useMemo(() => {
    ...
    // Handle special case for GWP intensity sorting
    if (sort.id === 'gwpIntensity') {
      return {
        [sort.desc ? 'dsc' : 'asc']: 'gwpIntensity',
      }
    }
    return {
      [sort.desc ? 'dsc' : 'asc']: sort.id,
  ...
  // Calculate GWP intensity for a project
  const calculateGwpIntensity = (row: any) => {
    const results = row.results
    const gwp = results?.gwp || {}
    const footprint = row.projectInfo?.grossFloorArea?.value || 0
    return footprint ? Number(gwp.total / footprint) : null
  }
  ...
  const columns = useMemo<MRT_ColumnDef<Pick<Contribution, 'id'>>[]>(
    () => [
      ...
      {
        // Add a virtual column for GWP intensity that we can filter and sort
        accessorKey: 'gwpIntensity',
        accessorFn: (row) => calculateGwpIntensity(row),
        Cell: ({ cell }) => {
          const value = cell.getValue<number>()
          return value ? value.toFixed(2) : 'N/A'
        },
  ...
  useEffect(() => {
    ...
    const filters = columnFilters.reduce<FilterAccumulator>(
      (acc, filter) => {
        ...
        // Special case for GWP Intensity
        if (fieldName === 'gwpIntensity') {
          // We need to handle this filter differently as it's a calculated field
          // For gwpIntensity, we need to filter based on results.gwp.total / projectInfo.grossFloorArea.value
          const [minValue, maxValue] = filter.value as [number, number]
          return {
            ...acc,
            gt: {
              ...acc.gt,
              gwpIntensity: minValue,
            },
            lt: {
              ...acc.lt,
              gwpIntensity: maxValue,
            },
            in: acc.in,
            equal: acc.equal,
            notEqual: acc.notEqual,
            contains: acc.contains,
          }
        }
