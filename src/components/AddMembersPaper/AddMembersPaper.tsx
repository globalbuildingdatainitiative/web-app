...
export const AddMembersPaper = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (data) {
        setInviteResults(data.inviteUsers.results)
        setEmails('')
      }
    } catch (err) {
        ...
        <Group>
          ...
          <Stack align='center' gap='xl'>
            ...
            <Textarea
              ...
              maxRows={5}
            />
            <Button color='green' radius='lg' px={16} size='md' style={{ width: '500px' }} type='submit' loading={loading}>
              Send Invitations
            </Button>
          </Stack>
        </Group>
      ...
      {inviteResults.length > 0 && (
        <Stack mt="md">
          ...
          {inviteResults.map((result, index) => (
            <Text key={index}>
              {result.email}: {result.status === 'invited' ? 'Invited successfully' : result.status === 'added' ? 'Added to organization' : result.message}
            </Text>
          ))}
        </Stack>
      )}
